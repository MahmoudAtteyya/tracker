      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 lg:h-20">
          ...
          <Link to="/" className="flex items-center gap-4 hover:opacity-90 transition-all duration-300 group">
            <div className={`p-3 rounded-2xl shadow-lg transition-all duration-300 group-hover:scale-110 ${isScrolled
            ...
            <div className="flex flex-col">
              <span className={`text-xl lg:text-2xl font-bold transition-all duration-300 ${isScrolled
                ? 'bg-gradient-to-r from-purple-600 to-indigo-600 bg-clip-text text-transparent'
                : 'text-white drop-shadow-lg'
                }`}>
                تتبع أوردرات إيلياء
              </span>
              <span className={`text-xs lg:text-sm transition-all duration-300 ${isScrolled ? 'text-gray-600 dark:text-gray-300' : 'text-white/90'
                }`}>
                Elliaa Tracking
              </span>
            </div>
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  key={item.path}
                  to={item.path}
                  className={`flex items-center gap-2 px-4 py-2 rounded-lg transition-all duration-300 hover:scale-105 ${isActive(item.path)
